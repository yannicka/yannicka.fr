...
E.create = function() {
	var width  = prompt('Largeur de la map ?');
	var height = prompt('Hauteur de la map ?');
...
E.draw = function() {
	...
	for (var y = by ; y < by + E.camera.bh + 1 ; y++) {
		for (var x = bx ; x < bx + E.camera.bw + 1 ; x++) {
			var tile = E.map[y][x];

			var basex = tile.g * E.basesize + tile.g;
			var basey = 0;

			var tile_by_line = Math.ceil(E.image.grounds.width / (E.basesize + 1));
			...
			if (typeof tile.n != 'undefined' && tile.n) {
				E.ctx.beginPath();
				E.ctx.fillStyle = rgba(255, 255, 0, .4);
				E.ctx.strokeStyle = rgba(0, 0, 0, .4);
				E.ctx.lineWidth = 2;
	...
	E.ctx_choiceblock.fillStyle = rgba(255, 0, 0, .4);
