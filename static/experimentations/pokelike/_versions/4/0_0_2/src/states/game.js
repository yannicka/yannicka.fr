...
G.game.update = function() {
	G.camera.x = Math.round(Math.clamp(-G.player.x + G.can.width / 4, (-G.map[0].length * G.basesize) + G.can.width / 2, 0));
	G.camera.y = Math.round(Math.clamp(-G.player.y + G.can.height / 4, (-G.map.length * G.basesize) + G.can.height / 2, 0));
...
G.game.draw = function() {
	...
	for (var y = 0 ; y < G.map_height ; y++) {
		for (var x = 0 ; x < G.map_width ; x++) {
			...
			var cc = Math.floor(G.image.grounds.width / G.basesize) - 1;
			var tile_by_line = Math.floor((G.image.grounds.width - cc) / G.basesize);
			...
			if (typeof tile.i != 'undefined') {
				var basex = tile.i * G.basesize + tile.i;
				var basey = 0;

				var cc = Math.floor(G.image.items.width / G.basesize) - 1;
				var tile_by_line = Math.floor((G.image.items.width - cc) / G.basesize);
	...
	G.ctx.drawImage(G.image.player, G.player.mov * 14, basey, 14, 18, G.player.x + 1, G.player.y - 1, 14, 18);

	G.ctx.restore();

	if (G.start_menu) {
		G.window_dialog(80, 0, G.can.width - 80, 86);
